...
Start:
    ...
    printf("Sending MCU EEPROM erase program\n");
    if (!WriteFile(hComm, eeerase, sizeof(eeerase), &send_num, NULL)) {
        printf("ERROR: sending MCU EEPROM erase\n");
        goto EXIT;
    }
...
#endif
                if (retry_num >= max_retry) {
                    printf("\nERROR: erase failed after %d attempts\n", retry_num);
                    goto EXIT;
                }
...
SAVE_FILE:
    ...
    if (hBuff == INVALID_HANDLE_VALUE) {
        printf("ERROR: creating MCU EEPROM backup file\n");
        goto EXIT;
    }

    if (!WriteFile(hBuff, recv_buffer, 0x200, &num, NULL)) {
        printf("ERROR: writing MCU EEPROM backup to file\n");
        goto EXIT;
    }
    ...
    printf("MCU EERPOM saved to %s\n", name_buffer);

//ConfirmErase

    for (int i = 0x200; i < 0x400; i++) {
        if (recv_buffer[i] != 0xFF) {
            printf("EEPROM erase failed @ 0x%02X\n", i);
            goto EXIT;
        }
    }
    printf("MCU EEPROM erase success\n");
    ret = 0;
