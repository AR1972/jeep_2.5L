...
Start:
	...
	if ((memcmp(flash_st + 1, recv_buffer, 0xA0) != 0) &
		(memcmp(flash_st + 1, recv_buffer + 1, 0xA0) != 0)) {
	...
	num = 0;
	send_num = 0;
	// currently the first time programming voltage is
	// applied we are getting a 0x00 byte in the write
	// buffer on the ECU, for now just toggle the 
	// programming voltage and have the bootstrap
	// discard the first sent byte. 
	for (int i = 0; i < 504; i++){
