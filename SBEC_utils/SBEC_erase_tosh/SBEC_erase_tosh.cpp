...
Start:
	...
	if ((memcmp(erase_tosh + 1, recv_buffer, 0x4F) != 0) &
		(memcmp(erase_tosh + 1, recv_buffer + 1, 0x4F) != 0)) {
		printf("Erase Tosh is corrupt\n");
		retry_num++;
		if (dev) {
			if (rel_onoff(dev, OFF, -4))
				goto EXIT;
